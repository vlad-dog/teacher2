...
client.on("ready", function() {
  setInterval(async () => {
    const statuslist = [
`with ${client.users.size} users | >help`,
    ...
    try {
      await client.user.setPresence({
        game: {
          ...
          type: "STREAMING",
          url: 'https://www.youtube.com/channel/UCWg2OR5mM3EbttXETepD1Gw'
        },
        status: "online"
      });
    } catch (error) {
...
client.on("guildMemberRemove", async member => {
  ...
  const Jimp = require("jimp")
    ...
    if (!canal) return;
        ...
        if (canal.type === "text") {            
                        const bg = await Jimp.read("https://cdn.discordapp.com/attachments/628314981452218368/689563928149360680/PicsArt_03-17-10.00.45.png");
            const userimg = await Jimp.read(member.user.avatarURL);
            var font;
            if (member.user.tag.length < 17) font = await Jimp.loadFont(Jimp.FONT_SANS_128_WHITE);
            else if (member.user.tag.length > 17) font = await Jimp.loadFont(Jimp.FONT_SANS_64_WHITE);
            else font = await Jimp.loadFont(Jimp.FONT_SANS_32_WHITE);
            await bg.print(font, 650, 230, member.user.tag);
            await userimg.resize(595, 575);
            await bg.composite(userimg, 43, 26).write("./img/"+ member.id + ".png");
              setTimeout(function () {
                    canal.send(new Discord.Attachment("./img/" + member.id + ".png"));
              }, 1000);
              setTimeout(function () {
...
client.on("guildMemberAdd", async member => {
  ...
  const Jimp = require("jimp")
    ...
    if (!canal) return;
        ...
        if (canal === undefined || canal === null) return;
        if (canal.type === "text") {            
                        const bg = await Jimp.read("https://cdn.discordapp.com/attachments/628314981452218368/689563732921155654/PicsArt_03-17-09.57.44.png")
            const userimg = await Jimp.read(member.user.avatarURL);
            var font;
            if (member.user.tag.length < 17) font = await Jimp.loadFont(Jimp.FONT_SANS_128_WHITE);
            else if (member.user.tag.length > 17) font = await Jimp.loadFont(Jimp.FONT_SANS_64_WHITE);
